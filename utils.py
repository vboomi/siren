...
def write_wave_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    writer.add_image(prefix + "pred_img", make_grid(pred, scale_each=False, normalize=True), global_step=total_steps)
...
def write_helmholtz_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    with torch.no_grad():
        ...
        if "pretrain" in gt:
            ...
            if torch.all(gt["pretrain"] == -1):
                gt["squared_slowness_grid"] = torch.clamp(pred[..., -1, None].clone(), min=-0.999) + 1.0
                gt["squared_slowness_grid"] = torch.where(
                    (torch.abs(coords[..., 0, None]) > 0.75) | (torch.abs(coords[..., 1, None]) > 0.75),
    ...
    writer.add_image(
        prefix + "pred_real", make_grid(pred[0::2, :, :, :], scale_each=False, normalize=True), global_step=total_steps
    )
    writer.add_image(
    ...
    writer.add_image(
        prefix + "pred_angle", make_grid(pred_angle, scale_each=False, normalize=True), global_step=total_steps
    )
    writer.add_image(
        prefix + "pred_mag", make_grid(pred_mag, scale_each=False, normalize=True), global_step=total_steps
    )
    ...
    if "gt" in gt:
        ...
        writer.add_image(
            prefix + "gt_angle", make_grid(gt_angle, scale_each=False, normalize=True), global_step=total_steps
        )
        writer.add_image(
            prefix + "gt_mag", make_grid(gt_mag, scale_each=False, normalize=True), global_step=total_steps
        )
    ...
    if "squared_slowness_grid" in gt:
        writer.add_image(
            ...
            make_grid(dataio.lin2img(gt["squared_slowness_grid"])[:2, :1], scale_each=False, normalize=True),
            global_step=total_steps,
        )
    ...
    if "img_sub" in model_input:
        writer.add_image(
            prefix + "img",
            make_grid(dataio.lin2img(model_input["img_sub"])[:2, :1], scale_each=False, normalize=True),
            global_step=total_steps,
        )
...
def write_image_summary_small(
    image_resolution, mask, model, model_input, gt, model_output, writer, total_steps, prefix="train_"
):
    if mask is None:
    ...
    with torch.no_grad():
        ...
        writer.add_image(
            prefix + "pred_grad_norm", make_grid(grad_norm, scale_each=False, normalize=True), global_step=total_steps
        )

    output_vs_gt = torch.cat((gt_img, pred_img), dim=-1)
    writer.add_image(
        prefix + "gt_vs_pred", make_grid(output_vs_gt, scale_each=False, normalize=True), global_step=total_steps
    )
...
def write_sdf_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_", device="cpu"):
    ...
    with torch.no_grad():
        ...
        xz_slice_coords = torch.cat(
            (slice_coords_2d[:, :1], torch.zeros_like(slice_coords_2d[:, :1]), slice_coords_2d[:, -1:]), dim=-1
        )
...
def write_video_summary(vid_dataset, model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    writer.add_image(
        prefix + "output_vs_gt", make_grid(output_vs_gt, scale_each=False, normalize=True), global_step=total_steps
    )
    min_max_summary(prefix + "coords", model_input["coords"], writer, total_steps)
...
def write_image_summary(image_resolution, model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    gt_img = dataio.lin2img(gt["img"], image_resolution)
    pred_img = dataio.lin2img(model_output["model_out"], image_resolution)
    ...
    pred_img = (
        dataio.rescale_img((pred_img + 1) / 2, mode="clamp").permute(0, 2, 3, 1).squeeze(0).detach().cpu().numpy()
    )
...
def write_laplace_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    writer.add_image(
        prefix + "comp_gt_vs_pred", make_grid(output_vs_gt, scale_each=False, normalize=True), global_step=total_steps
    )
    ...
    writer.add_image(
        prefix + "pred_grad", make_grid(grads_img, scale_each=False, normalize=True), global_step=total_steps
    )

    # Plot gt image
    writer.add_image(prefix + "gt_img", make_grid(gt_img, scale_each=False, normalize=True), global_step=total_steps)
    ...
    # Plot pred image
    writer.add_image(
        prefix + "pred_img", make_grid(pred_img, scale_each=False, normalize=True), global_step=total_steps
    )
    ...
    writer.add_image(
        prefix + "pred_grad", make_grid(pred_grads_img, scale_each=False, normalize=True), global_step=total_steps
    )
...
def write_gradients_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    # Plot gt gradient
    writer.add_image(
        prefix + "gt_grad", make_grid(gt_grads_img, scale_each=False, normalize=True), global_step=total_steps
    )
...
def write_gradcomp_summary(model, model_input, gt, model_output, writer, total_steps, prefix="train_"):
    ...
    writer.add_image(
        prefix + "gt_grads1", make_grid(gt_grads1_img, scale_each=False, normalize=True), global_step=total_steps
    )
    writer.add_image(
        prefix + "gt_grads2", make_grid(gt_grads2_img, scale_each=False, normalize=True), global_step=total_steps
    )

    writer.add_image(
        prefix + "gt_gradcomp", make_grid(gt_grads_img, scale_each=False, normalize=True), global_step=total_steps
    )
    writer.add_image(
        prefix + "pred_gradcomp", make_grid(pred_grads_img, scale_each=False, normalize=True), global_step=total_steps
    )
    # Plot gt image
    gt_img1 = dataio.lin2img(gt["img1"])
    gt_img2 = dataio.lin2img(gt["img2"])
    writer.add_image(prefix + "gt_img1", make_grid(gt_img1, scale_each=False, normalize=True), global_step=total_steps)
    writer.add_image(prefix + "gt_img2", make_grid(gt_img2, scale_each=False, normalize=True), global_step=total_steps)

    # Plot pred compo image
    pred_img = dataio.rescale_img(dataio.lin2img(model_output["model_out"]))
    writer.add_image(
        prefix + "pred_comp_img", make_grid(pred_img, scale_each=False, normalize=True), global_step=total_steps
    )
